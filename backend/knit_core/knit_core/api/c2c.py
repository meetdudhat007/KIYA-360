...
ADR-004 names UX/Mobile as one of eight KNIT-owned strategic seams. A front end
...
front end would be coupled to the framework rather than to KNIT.
...
from knit_core.business_status import engine, model
from knit_core.crm import conversion
from knit_core.sales import quotation_from
...
def _line_table(doc):
	...
	Read from the meta rather than listed here, so a column added to KNIT
...
def create_lead(
	...
	knit_business_status or any other field the seam does not offer."""
	doc = frappe.get_doc(
		{
			"doctype": "KNIT Lead",
			"lead_name": lead_name,
...
def companies():
	...
	return frappe.get_all("KNIT Company", fields=["name", "default_currency"], order_by="name")
