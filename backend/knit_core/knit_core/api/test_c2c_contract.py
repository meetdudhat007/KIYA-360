...
Runs without a site:
    python -m unittest knit_core.api.test_c2c_contract

knit_core.api.c2c imports frappe, so this reads the module's STAGES table out of
...
from knit_core.business_status import model  # noqa: E402  (after sys.path setup)
...
class TestC2CContract(unittest.TestCase):
	...
	def test_stages_are_in_flow_order(self):
		...
		self.assertEqual(
			...
			["KNIT Lead", "KNIT Opportunity", "KNIT Quotation"],
		)
	...
	def test_conversion_target_status_is_in_the_lead_lifecycle(self):
		...
		lifecycle = model.REGISTRY["KNIT Lead"]
