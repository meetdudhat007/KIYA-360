"""Structural tests for the knit_core doctype definitions.

Runs without a site:
    python -m unittest knit_core.platform.test_platform_doctypes

The test that matters is test_link_targets_are_bare_frappe_safe. knit_core must
...
# Frappe framework doctypes knit_core is allowed to depend on.
...
class TestDoctypeDefinitions(unittest.TestCase):
	...
	def test_link_targets_are_bare_frappe_safe(self):
		"""knit_core must not depend on ERPNext or HRMS."""
		for path in self.files:
			d = load(path)
			for f in d["fields"]:
				if f["fieldtype"] != "Link":
				...
				self.assertNotIn(
					...
					f"knit_core must install on bare Frappe.",
				)
				self.assertTrue(
