...
model.py imports nothing from frappe, so these run without a site:
    ...
    python -m unittest knit360_core.business_status.test_business_status
...
class TestLifecycleRegistry(unittest.TestCase):
	...
	def test_registered_doctypes_get_their_own(self):
		self.assertIs(model.for_doctype("KNIT 360 Lead"), model.LEAD)
		self.assertIs(model.for_doctype("KNIT 360 Opportunity"), model.OPPORTUNITY)
