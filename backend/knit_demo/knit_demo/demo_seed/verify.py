"""Read-only green/red check on the demo state.
    ...
    bench --site <site> execute knit_demo.demo_seed.verify.run
...
from knit_core.business_status import model as bs_model
from knit_core.business_status.engine import FIELD as BS_FIELD
from knit_core.business_status.engine import LOG_DOCTYPE, conflicting_workflow
from knit_core import brd_data
from knit_demo.demo_seed import platform
...
@check("knit_core custom fields")
def _custom_fields():
	missing = []
	for doctype, fieldname in (
		("Role", "knit_approval_authority"),
		("Item", "knit_hsn_sac_code"),
	):
	...
	authority = frappe.db.get_value(
		"Role", brd_data.BRD_USER_ROLE["role_name"], "knit_approval_authority"
	)
	if not authority:
...
def _item():
	...
	hsn = frappe.db.get_value("Item", item["item_code"], "knit_hsn_sac_code")
