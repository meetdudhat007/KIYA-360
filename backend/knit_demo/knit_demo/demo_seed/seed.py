"""Idempotent demo seeding for the KNIT prototype.

Run:
    bench --site <site> execute knit_demo.demo_seed.seed.execute
...
from knit_demo.demo_seed import custom_fields
from knit_demo.demo_seed._util import assert_fields as _assert_fields
from knit_demo.demo_seed._util import insert_if_absent as _insert
...
def seed_dev2():
	...
	_assert_fields("Item", ["item_code", "item_name", "item_group", "stock_uom", "knit_hsn_sac_code"])
	_insert(
		"Item",
		{"item_code": item["item_code"]},
		{
			...
			"knit_hsn_sac_code": item["hsn_sac_code"],
		},
	)
...
def set_role_approval_authority():
	...
	frappe.db.set_value(
		"Role", role["role_name"], "knit_approval_authority", role["approval_authority"]
	)
