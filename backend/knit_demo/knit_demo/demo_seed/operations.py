...
Run after seed.execute():
    bench --site <site> execute knit_demo.demo_seed.operations.run_all
"""
...
from knit_demo.demo_seed import flows
from knit_demo.demo_seed._util import assert_fields, insert_if_absent, require, submit_new
...
def _employee():
	...
	if not name:
		frappe.throw("Demo Employee not found. Run knit_demo.demo_seed.seed.execute first.")
	return name
...
def seed_production():
	...
	require("Item", finished, "Run knit_demo.demo_seed.seed.execute first.")
...
def seed_asset_and_quality():
	...
	require(
		"Asset Category", asset_category, "Run knit_demo.demo_seed.seed.execute first."
	)
