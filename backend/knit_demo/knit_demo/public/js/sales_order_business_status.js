...
frappe.provide("knit.business_status");
...
knit.business_status.headline = function (frm, status) {
...
knit.business_status.confirm = function (frm, next) {
	frappe.prompt(
		[
			{
				fieldname: "reason",
		...
		(values) => {
			frappe.call({
				method: "knit_core.business_status.engine.transition",
				args: {
...
knit.business_status.render = function (frm) {
	const status = frm.doc.knit_business_status;
	if (!status) return;

	knit.business_status.headline(frm, status);

	frappe.call({
		method: "knit_core.business_status.engine.allowed_next",
		...
		callback: (r) => {
			...
			next_states.forEach((next) => {
				frm.add_custom_button(
					__(next),
					() => knit.business_status.confirm(frm, next),
					__("Business Status")
				);
			});
		},
...
frappe.ui.form.on("Sales Order", {
	refresh(frm) {
		...
		knit.business_status.render(frm);
	},
});
