"""kiya_core custom fields.

Both fields exist because the BRD requires data core ERPNext has no field for:

- Role.kiya_approval_authority -- BRD FR-PADM-1.2.4 (Approval Authority).
  Frappe's Role doctype has no monetary approval ceiling.

- Item.kiya_hsn_sac_code -- BRD Section 1.4 Item Master ("item code, category,
  ...
  installed. Tax & Statutory Compliance is a KIYA-owned seam, so the field is
  owned here.

Fields are defined in kiya_core only; no ERPNext or Frappe file is modified.
"""
...
CUSTOM_FIELDS = {
	"Role": [
		{
			"fieldname": "kiya_approval_authority",
...
def install():
	"""Create or update the kiya_core custom fields. Idempotent."""
